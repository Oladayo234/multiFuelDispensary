...
def add_fuel_to_inventory(attendant):
    try:
        fuel_type = input("Enter fuel type (petrol/diesel/kerosene/gas): ")
...
def restock_fuel(attendant, dispenser):
    try:
        fuel_type = input("Enter fuel type to restock: ")
...
def view_sales_records(attendant):
    ...
    for i, sale in enumerate(sales, 1):
        print(f"{i}. {sale['fuel_type'].capitalize()} - {sale['liters']:.2f}L - N{sale['total']:.2f}")
...
def view_all_transactions(dispenser):
    try:
        ...
        for i, trans in enumerate(transactions, 1):
            print(f"{i}. {trans['fuel_type'].capitalize()} - {trans['liters']:.2f}L - N{trans['total']:.2f}")
        print("============================")
