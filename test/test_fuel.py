...
class TestFuel(TestCase):
    ...
    def test_that_fuel_can_be_created_with_correct_inputs(self):
    ...
    def test_that_negative_quantity_on_creation_raises_error(self):
        with self.assertRaises(ValueError):
            Fuel(quantity=-10.0, fuel_type="petrol", price_per_liter=1.50)

    def test_that_invalid_fuel_type_on_creation_raises_error(self):
        with self.assertRaises(ValueError):
            Fuel(quantity=50.0, fuel_type="Gasoline", price_per_liter=1.50)

    def test_that_zero_price_on_creation_raises_error(self):
        with self.assertRaises(ValueError):
            Fuel(quantity=50.0, fuel_type="petrol", price_per_liter=0.0)
    ...
    def test_that_fuel_type_is_stripped_of_whitespace(self):
        self.fuel.set_fuel_type("  Diesel  ")
        self.assertEqual(self.fuel.get_fuel_type(), "diesel")
